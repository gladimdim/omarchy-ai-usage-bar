...
- Plugin ID: `gladimdim.ai-limits`
- Display name: AI Usage Bar
- Repository: https://github.com/gladimdim/omarchy-ai-usage-bar
- Default branch: `master`
- Listing: https://plugins.omarchy.org/plugin.html?id=gladimdim.ai-limits

## After every published release

Once the release commit is on `origin/master` (tag pushed, GitHub release published, or `master` fast-forwarded):

1. Confirm `gh` is authenticated as `gladimdim`.
2. `git fetch origin` and take the full 40-character SHA from `git rev-parse origin/master`.
...
import json, sys
want, head = 'gladimdim.ai-limits', sys.argv[1]
...
https://github.com/gladimdim/omarchy-ai-usage-bar
...
gh issue create \
  ...
  --title "[Verify]: AI Usage Bar v${VERSION}" \
